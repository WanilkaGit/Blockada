...
def collides():# доторкання
    ...
    if sprite.collide_rect(hero, p8) and open_d == False:
        hero.rect.x = door.rect.x - hero.level1_width
        window.blit(k_need, (450, 50))

    if sprite.collide_rect(hero, p8) and open_d == True:
        window.blit(e_tap, (450, 50))
        if key_pressed(K_e):
            p8.rect.x = -200
            p8.image = transform.scale(image.load(chest_open), (p8.width, p8.height)
            open_d = True
...
def start_pos():# стартова позиція
    global items, camera, hero, block_r, block_l, plat, coins
    global stairs_lst, enemy_lst, p6, p11, p7, p8, open_d, open_ch# робимо глобальними змінни
    ...
    for r in level1:# фор як раб почав ходити по списками перевіряєм індекси
        for c in r:#  стучим в двері перевіряєм чи
            ...
            if c == "g":
                ...
                items.add(p7)
            if c == "d":
                p8 = Settings(x,y, 40, 80, 0, door_img)#  ключа
                items.add(p8)
            if c  == "e":
